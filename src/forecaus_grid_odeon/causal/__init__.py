...
from .graph import OUTCOME, TREATMENT, build_dag, draw_dag, to_dot
from .refute import refute

__all__ = [
    "build_dag", "draw_dag", "to_dot", "estimate_effect", "refute", "run_causal",
    "TREATMENT", "OUTCOME",
]


def run_causal(df: pd.DataFrame, treatment: str = TREATMENT, outcome: str = OUTCOME,
               random_seed: int = 0, num_simulations: int = 100) -> tuple[dict, pd.DataFrame]:
    """Estimate the effect and run all refuters. Returns (result, refute_table)."""
    result = estimate_effect(df, treatment, outcome)
